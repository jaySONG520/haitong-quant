...
function handleSecuritySearchInput() {
  ...
  // 在本地全量 ETF 映射表中查找（支持静态 HTML 模式）
  if (query.length >= 2) {
    const localMatch = searchFullEtfMap(query);
    if (localMatch) {
      state.externalLookupResult = localMatch;
      state.selectedSymbol = localMatch.symbol;
      state.lookupError = "";
      return;
    }
  }
  ...
  if (state.dataSource !== "real" || !isServiceMode() || query.length < 2) {
    state.externalLookupResult = null;
    state.lookupError = query.length >= 2 ? "当前候选池中没有匹配标的。可更新配置文件扩大股票池后重新运行 pipeline。" : "";
    return;
  }
...
// 辅助数据填充函数
// 全量 ETF 名称 + 跟踪指数映射表（用于搜索和展示）
const FULL_ETF_MAP = {
  // 宽基指数 ETF
  "510300": { name: "沪深300ETF", index: "沪深300" },
  "510050": { name: "上证50ETF", index: "上证50" },
  "159915": { name: "创业板ETF", index: "创业板指" },
  "512100": { name: "中证1000ETF南方", index: "中证1000" },
  "159922": { name: "中证500ETF", index: "中证500" },
  "510500": { name: "中证500ETF南方", index: "中证500" },
  "518880": { name: "黄金ETF华安", index: "黄金现货" },
  "159601": { name: "A50ETF", index: "富时中国A50" },
  "588000": { name: "科创50ETF", index: "科创50" },
  "159919": { name: "沪深300ETF华安", index: "沪深300" },
  // 行业/主题 ETF
  "512480": { name: "半导体ETF", index: "中证半导体" },
  "512660": { name: "军工ETF", index: "中证军工" },
  "512010": { name: "医药ETF", index: "中证医药" },
  "159928": { name: "消费ETF", index: "中证消费" },
  "515030": { name: "新能源车ETF", index: "CS新能车" },
  "512690": { name: "酒ETF", index: "中证白酒" },
  "512000": { name: "券商ETF", index: "中证全指证券" },
  "515880": { name: "通信ETF", index: "中证全指通信" },
  "512800": { name: "银行ETF", index: "中证银行" },
  "159869": { name: "游戏ETF", index: "中证动漫游戏" },
  "516160": { name: "新能源ETF", index: "CS新能源" },
  "515790": { name: "光伏ETF", index: "中证光伏产业" },
  "512980": { name: "传媒ETF", index: "中证传媒" },
  "515050": { name: "5GETF", index: "中证5G通信" },
  "159941": { name: "纳指ETF", index: "纳斯达克100" },
  ...
  "512170": { name: "医疗ETF", index: "中证医疗" },
  "159766": { name: "旅游ETF", index: "中证旅游" },
  "512200": { name: "房地产ETF", index: "中证房地产" },
  "512400": { name: "有色金属ETF", index: "中证有色" },
  "512880": { name: "证券ETF", index: "中证全指证券" },
  "159736": { name: "半导体设备ETF", index: "半导体设备" },
  "515220": { name: "煤炭ETF", index: "中证煤炭" },
  "159611": { name: "电力ETF", index: "中证电力" },
  "516110": { name: "汽车ETF", index: "中证汽车" },
  "562800": { name: "中证2000ETF", index: "中证2000" },
  "159852": { name: "机器人ETF", index: "中证机器人" },
  "159819": { name: "人工智能ETF", index: "中证人工智能" },
  "588200": { name: "科创芯片ETF", index: "科创芯片" },
  "513100": { name: "纳指ETF国泰", index: "纳斯达克100" },
  "513050": { name: "中概互联ETF", index: "中国互联网50" },
  "513500": { name: "标普500ETF", index: "标普500" },
  "159740": { name: "恒生科技ETF", index: "恒生科技" },
  "513180": { name: "恒生科技ETF华安", index: "恒生科技" },
  "159605": { name: "中概互联ETF易方达", index: "中国互联网50" },
};

function getFundName(symbol) {
  const entry = FULL_ETF_MAP[symbol];
  return entry ? entry.name : "量化精选ETF";
}

// 在全量 ETF 映射表中搜索（支持中文名称/跟踪指数/代码模糊匹配）
function searchFullEtfMap(query) {
  const q = query.toLowerCase();
  for (const [symbol, info] of Object.entries(FULL_ETF_MAP)) {
    const haystack = `${symbol} ${info.name} ${info.index}`.toLowerCase();
    if (haystack.includes(q)) {
      // 检查是否已在候选池中
      const data = getActiveData();
      const inPool = data.candidates.some(c => c.symbol === symbol);
      if (inPool) return null; // 交给 getFilteredCandidates 处理
      return {
        symbol: symbol,
        name: info.name,
        ...
        risk_level: "未评级",
        date: "",
        news_summary: `${info.name}（${info.index}）当前不在配置的候选池中。如需跟踪决策信号，请将 ${symbol} 加入 configs/default.json 的 symbols 列表后运行 pipeline 命令。`,
        advantages: ["已找到该ETF信息"],
        risks: ["当前不在候选池中，需更新配置并运行 pipeline 才能生成交易计划"],
        is_external_lookup: true,
      };
    }
  }
